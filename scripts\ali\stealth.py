...
(() => {
    ...
    // 7. WebGL vendor/renderer — 与平台一致（同时覆盖 WebGL1 和 WebGL2）
    ...
    // 8. hardwareConcurrency — 随机 4 或 8
    Object.defineProperty(navigator, 'hardwareConcurrency', {
        get: () => [4, 8][Math.floor(Math.random() * 2)],
        configurable: true,
    });

    // 9. deviceMemory — 随机 4 或 8
    Object.defineProperty(navigator, 'deviceMemory', {
        get: () => [4, 8][Math.floor(Math.random() * 2)],
        configurable: true,
    });

    // 10. navigator.connection — 伪造网络信息
    ...
    // 11. chrome.csi / chrome.loadTimes — 空函数伪装
    if (window.chrome) {
